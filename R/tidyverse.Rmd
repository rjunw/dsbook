...
b.  `ChickWeight` is tidy: each observation (a weight) is represented by one row. The chick from which this measurement came is one of the variables. **(Correct)**
...
c.  `BOD` is tidy: each row is an observation with two values (time and demand) **(Correct)**
d.  `BOD` is tidy: all small datasets are tidy by definition.

4\. Which of the following built-in datasets is tidy (you can pick more than one): *After `data.frame(…)`*

a.  `BJsales` **(Correct)**
b.  `EuStockMarkets`
c.  `DNase` **(Correct)**
d.  `Formaldehyde` **(Correct)**
e.  `Orange` **(Correct)**
f.  `UCBAdmissions` **(Correct)**
...
```{r}
murders_nw |> filter(rate < 1) |> select(state, rate, rank)
```
...
b.  `murders` is in tidy format and is stored in a data frame. **(Correct)**
