...
[^random-variables-sampling-models-clt-2]: [https://en.wikipedia.org/w/index.php?title=Security\_(finance)](https://en.wikipedia.org/w/index.php?title=Security_(finance)){.uri}
...
```{r}
set.seed(1)
S = sum(sample(c(17,-1), prob = c(1/19, 18/19), size = 1000, replace = T))
S
```
...
```{r}
1 - pnorm(0, muS, sqrt(vS)) # 1 - Prob(S < 0)
```
...
```{r}
set.seed(1)
S = vector(mode="numeric", length=1000000)
for (i in 1:1000000){
    S[i] = sum(sample(c(17,-1), prob = c(1/19, 18/19), size = 1000, replace = T))
}
mean(S)
sqrt(var(S))
```
...
a.  1,000 simulations is not enough. If we do more, they match. **(Correct)**
...
for (i in 1:1000){
    Y[i] = mean(sample(c(17,-1), prob = c(1/19, 18/19), size = 1000, replace = T))
}
mean(Y)
```
...
for (i in 1:n){
    Y[i] = mean(sample(c(17,-1), prob = c(1/19, 18/19), size = n, replace = T))
}
```
